...
class EnsembleService:
    ...
    def __init__(self):
        ...
        try:
            ...
            raise Exception(f"Ensemble Data client initialization failed: {str(e)}")
    ...
    async def get_profile(self, username_or_url: str) -> TikTokProfile:
        ...
        try:
            ...
            if units_charged:
                logger.info(f"💰 Ensemble units charged (profile): {units_charged}")
            
            # Extract profile data from official response structure
            profile_data = response.data if hasattr(response, 'data') else response
            user_data = profile_data.get("user", {}) if isinstance(profile_data, dict) else {}
            stats_data = profile_data.get("stats", {}) if isinstance(profile_data, dict) else {}
            ...
            raise Exception(f"Could not retrieve profile for @{username}: API error - {str(e)}")
        except Exception as e:
            logger.error(f"❌ Failed to fetch profile for @{username}: {e}")
            raise Exception(f"Could not retrieve profile for @{username}: {str(e)}")
    ...
    async def get_posts(
        ...
        try:
            ...
            depth = min((count + 9) // 10, 5)  # Max depth 5 as per best practices
            ...
            # Call official SDK method: tiktok_user_posts_from_username (without .tiktok)
            response = await self._run_in_executor(
                self.client.tiktok_user_posts_from_username,
                **params
            )
            ...
            units_charged = getattr(response, 'units_charged', 0)
            if units_charged:
                logger.info(f"💰 Ensemble units charged (posts): {units_charged}")
            
            # Extract posts data from official response structure
            posts_data = response.data if hasattr(response, 'data') else response
            posts_list = posts_data.get("data", []) if isinstance(posts_data, dict) else []
            next_cursor = posts_data.get("nextCursor") if isinstance(posts_data, dict) else None
            ...
            if "rate limit" in error_msg or "429" in error_msg:
                raise Exception(f"API rate limit exceeded for @{username}. Please try again in a few minutes.")
            elif "not found" in error_msg or "404" in error_msg:
                raise Exception(f"TikTok profile @{username} not found or is private/restricted.")
            elif "forbidden" in error_msg or "403" in error_msg:
                raise Exception(f"Access denied to @{username}. This profile may be protected or geo-restricted.")
            else:
                raise Exception(f"Unable to retrieve posts for @{username}. API error: {str(e)}")
        except Exception as e:
            logger.error(f"❌ Failed to fetch posts for @{username}: {e}")
            raise Exception(f"Unable to retrieve posts for @{username}. Error: {str(e)}")
    ...
    async def search_hashtag_posts(
        ...
        logger.info(f"🔍 Searching posts for hashtag: #{clean_hashtag} (period: {period} days)")
        ...
        try:
            ...
            if period > 0 and cursor is None:
                try:
                    ...
                    units_charged = getattr(response, 'units_charged', 0)
                    if units_charged:
                        logger.info(f"💰 Ensemble units charged (full hashtag): {units_charged}")
                    
                    # Extract data from full search response
                    hashtag_data = response.data if hasattr(response, 'data') else response
                    posts_list = hashtag_data.get("data", []) if isinstance(hashtag_data, dict) else []
                    
                except Exception as full_search_error:
                    logger.info(f"⚠️ Full hashtag search failed, fallback to regular search: {full_search_error}")
            ...
            logger.debug(f"📊 Raw posts found: {len(posts)} for #{clean_hashtag}")
            if posts:
                logger.debug(f"📈 Sample post views: {[p.views for p in posts[:3]]}")
            ...
            if not quality_posts and posts:
                logger.info(f"🔍 No posts > 100 views, using lower threshold for #{clean_hashtag}")
                quality_posts = [post for post in posts if post.views > 10]
            
            final_posts = quality_posts[:count] if quality_posts else posts[:count]
            logger.info(f"✅ Found {len(final_posts)} quality posts for #{clean_hashtag}")
            
            return final_posts
            
        except EDError as e:
            logger.error(f"❌ Ensemble API error for hashtag #{clean_hashtag}: {e}")
            error_msg = str(e).lower()
            
            # Provide specific error messages for hashtag search
            if "rate limit" in error_msg or "429" in error_msg:
                raise Exception(f"API rate limit exceeded for hashtag #{clean_hashtag}. Please try again later.")
            elif "not found" in error_msg or "404" in error_msg:
                raise Exception(f"No content found for hashtag #{clean_hashtag} or hashtag is restricted.")
            elif "forbidden" in error_msg or "403" in error_msg:
                raise Exception(f"Access denied to hashtag #{clean_hashtag}. Content may be geo-restricted.")
            else:
                raise Exception(f"Unable to search hashtag #{clean_hashtag}. API error: {str(e)}")
        except Exception as e:
            logger.error(f"❌ Failed to search hashtag #{clean_hashtag}: {e}")
            raise Exception(f"Unable to search hashtag #{clean_hashtag}. Error: {str(e)}")
    ...
    async def _search_hashtag_with_cursor(
        ...
        while len(all_posts) < count and attempts < max_attempts:
            try:
                ...
                units_charged = getattr(response, 'units_charged', 0)
                if units_charged:
                    logger.info(f"💰 Ensemble units charged (hashtag page {attempts+1}): {units_charged}")
                
                # Extract data from response
                page_data = response.data if hasattr(response, 'data') else response
                posts_batch = page_data.get("data", []) if isinstance(page_data, dict) else []
                next_cursor = page_data.get("nextCursor") if isinstance(page_data, dict) else None
                ...
                logger.warning(f"⚠️ Error in hashtag search page {attempts+1}: {e}")
                break
    ...
    async def search_users(self, query: str, count: int = 10) -> List[TikTokProfile]:
        ...
        logger.info(f"🔍 Consider using alternative approaches like searching specific usernames directly")
    ...
    def _parse_profile_data(
        ...
        if not isinstance(user_data, dict):
            logger.warning(f"Invalid user data format for @{username}: expected dict, got {type(user_data)}")
            user_data = {}
        
        if not isinstance(stats_data, dict):
            logger.warning(f"Invalid stats data format for @{username}: expected dict, got {type(stats_data)}")
            stats_data = {}
        ...
        if not isinstance(extracted_username, str):
            extracted_username = str(extracted_username) if extracted_username else username
        ...
        logger.debug(f"📋 Parsed profile @{extracted_username}: followers={follower_count}, videos={video_count}")
    ...
    def _parse_posts_data(self, posts_list: List[Dict[str, Any]], count: int) -> List[TikTokPost]:
        ...
        if not isinstance(posts_list, list):
            logger.warning(f"Invalid posts data format: expected list, got {type(posts_list)}")
            return []
        ...
        for i, post_data in enumerate(posts_list[:count]):
            try:
                if not isinstance(post_data, dict):
                    logger.warning(f"Invalid post data at index {i}: expected dict, got {type(post_data)}")
                    continue
                ...
                video_url = (
                    ...
                    safe_get_nested(video_info, ['download_addr', 'url_list', 0]) or ""
                )
                ...
                post = TikTokPost(
                    ...
                    video_url=video_url[:500] if isinstance(video_url, str) else "",
                    cover_image_url=cover_image_url[:500] if isinstance(cover_image_url, str) else "",
                    hashtags=hashtags[:10]  # Limit hashtags for safety
                )
                posts.append(post)
                
            except Exception as e:
                logger.warning(f"⚠️ Failed to parse post data at index {i}: {e}")
                continue
        
        logger.debug(f"📱 Successfully parsed {len(posts)} posts from {len(posts_list)} raw posts")
        return posts
    ...
    def _parse_hashtag_posts(
        ...
        Returns:
            List of TikTokPost objects
        """
        logger.debug(f"🔍 Parsing hashtag posts for #{hashtag}: {len(posts_list) if isinstance(posts_list, list) else 0} posts")
