...
class Settings(BaseSettings):
    ...
    debug: bool = Field(default=True, env="DEBUG")  # Enable debug by default
    ...
    cache_profile_ttl: int = Field(default=1800, env="CACHE_PROFILE_TTL")  # 30 min
    cache_posts_ttl: int = Field(default=900, env="CACHE_POSTS_TTL")      # 15 min
    cache_trends_ttl: int = Field(default=300, env="CACHE_TRENDS_TTL")    # 5 min
    ...
    ensemble_max_depth: int = 5  # Max depth for user posts (each depth ≈ 10 posts)
    ...
    def validate_ensemble_token(cls, v):
        ...
        if not token.replace('-', '').replace('_', '').isalnum():
            logger.warning("⚠️ Ensemble token format may be invalid (contains special characters)")
