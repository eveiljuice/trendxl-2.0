...
async def create_user(email: str, username: str, password: str, full_name: Optional[str] = None) -> Dict[str, Any]:
    """Legacy wrapper for register_user"""
...
async def get_user_by_username(username: str) -> Optional[Dict[str, Any]]:
    """Get user by username from Supabase profiles table"""
    try:
        client = get_supabase()
...
async def record_token_usage(user_id: str, endpoint: str, tokens_used: int, model: str = "gpt-4", cost: float = 0.0) -> bool:
...
async def get_user_token_usage(user_id: str, limit: int = 50, offset: int = 0) -> list:
...
async def get_user_token_summary(user_id: str) -> Dict[str, Any]:
    ...
    try:
        ...
        total_tokens = sum(r.get("tokens_used", 0) for r in response.data)
        total_cost = sum(r.get("cost", 0.0) for r in response.data)
...
async def get_user_token_usage_by_period(user_id: str, period_days: int = 30) -> Dict[str, Any]:
    ...
    try:
        ...
        return {
            ...
            "total_cost": total_cost,
            "request_count": len(response.data),
            "daily_average": total_tokens / period_days if period_days > 0 else 0
        }
    except Exception as e:
        logger.error(f"Error getting token usage by period: {e}")
        return {
            "period_days": period_days,
            "total_tokens": 0,
            "total_cost": 0.0,
